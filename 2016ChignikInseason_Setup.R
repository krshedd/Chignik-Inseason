...
## Kyle Shedd Wed Jun 24 17:00:22 2015
...
# Create file structure based on 2014
dirs2014 <- list.dirs(path = "V:/WORK/Sockeye/Chignik/2014 Chignik Inseason/Kyle", recursive = FALSE, full.names = FALSE)
sapply(dirs2014, dir.create)
sapply(c("Control", "Mixture", "Output"), function(folder) {dir.create(path = paste(getwd(), "BAYES", folder, sep = "/"))})
dir.create(path = "Contacts and schedule")

## save.image("2015/2015ChignikInseason.RData")
## load("2015/2015ChignikInseason.RData")
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Black2010to2014Means <- dget(file="V:/WORK/Sockeye/Chignik/2014 Chignik Inseason/Kyle/Objects/Black2010to2014Means.txt")
Black2010to2014Uppers <- dget(file="V:/WORK/Sockeye/Chignik/2014 Chignik Inseason/Kyle/Objects/Black2010to2014Uppers.txt")
Black2010to2014Lowers <- dget(file="V:/WORK/Sockeye/Chignik/2014 Chignik Inseason/Kyle/Objects/Black2010to2014Lowers.txt")

Black2010to2015Means <- rbind(Black2010to2014Means, "2015" = rep(NA, 6))
dput(x = Black2010to2015Means, file = "2015/Objects/Black2010to2015Means.txt")
Black2010to2015Uppers <- rbind(Black2010to2014Uppers, "2015" = rep(NA, 6))
dput(x = Black2010to2015Uppers, file = "2015/Objects/Black2010to2015Uppers.txt")
Black2010to2015Lowers <- rbind(Black2010to2014Lowers, "2015" = rep(NA, 6))
dput(x = Black2010to2015Lowers, file = "2015/Objects/Black2010to2015Lowers.txt")

# This is a new object for this year
GeneticEstimates2015 <- matrix(data = NA, nrow = 6, ncol = 5, dimnames = list(1:6, c("Day", "BlackMean", "5%", "95%", "sd")))
dput(x = GeneticEstimates2015, file = "2015/Objects/GeneticEstimates2015.txt")
...
# "2015/averagemodel_fromBirch20150624.xlsx"
...
dput(x = birch.lwr, file = "2015/Objects/birch.lwr.txt")
dput(x = birch.upr, file = "2015/Objects/birch.upr.txt")

## THD modifying on 062615
birch.lwr <- dget(file = "2015/Objects/birch.lwr.txt")
birch.upr <- dget(file = "2015/Objects/birch.upr.txt")


#Test ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
NewData <- dget(file = "2015/Estimates objects/SCHIG15_1_Jun27_Estimates.txt")
Period = 1
NumSampled = 190
NumAnalyzed = 190
Included = 190
...
ChignikInseasonReport.f <- function(NewData = XX, Period = period, NumSampled = sampled, NumAnalyzed = anlayzed, Included = 190, Month = "June", Day = 28) {
  ...
  GeneticEstimates2015[Period, ] <- c(day, NewData[[1]][[1]][1, c("mean", "5%", "95%", "sd")]) 
  ...
  Year <- rev(rownames(Black2010to2015Means))[1]
  
  ## Adding this set of estimates to summary object
  Black2010to2015Means[6, Period] <- NewData$Stats[[1]][1, "mean"]
  Black2010to2015Uppers[6, Period] <- NewData$Stats[[1]][1, "95%"]
  Black2010to2015Lowers[6, Period] <- NewData$Stats[[1]][1, "5%"]
  
  wrapper <- dget(file = "V:/WORK/Sockeye/Chignik/2013 Chignik Inseason/Objects/wrapper.txt")
  
  layoutmatrix2 <- dget(file = "V:/WORK/Sockeye/Chignik/2013 Chignik Inseason/Objects/layoutmatrix2.txt")
  
  layout(layoutmatrix2, widths = c(0.3, 1, 0.3), heights = c(0.75, 0.25, 1, 0.1), respect = FALSE)
  #layout.show(4)
  
  pdf(paste("Updates/2015ChignikInseason", Period, ".pdf", sep = ""), family = "Times", width = 8.5, height = 11, title = paste(Year, " Chignik Inseason", Period, sep = ""))
  ...
  legend("topright", legend = c("Average\ntransition\n2010-2014", "2015"), pch = c(22, 21), col = c("grey70", "black"), pt.bg=c("grey70", "red"),cex = 1.2, bty = "n", pt.cex = 2)  ## pt.cex from 3 to 2; pch from 16 to 21; THD on 062615
  ...
  dput(x = GeneticEstimates2015, file = "2015/Objects/GeneticEstimates2015.txt")
  dput(Black2010to2015Means, file = "2015/Objects/Black2010to2015Means.txt")
  dput(Black2010to2015Uppers, file = "2015/Objects/Black2010to2015Uppers.txt")
  dput(Black2010to2015Lowers, file = "2015/Objects/Black2010to2015Lowers.txt")
...
SCHIG15_1_Jun27_Estimates_TEST <- dget(file = "V:/WORK/Sockeye/Chignik/2014 Chignik Inseason/Kyle/Estimates objects/SCHIG14_1_Jun28_Estimates.txt")
...
# Save function
dput(ChignikInseasonReport.f, file = "2015/Objects/ChignikInseasonReport.f.txt")


#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#### Making a formula to put out report OLD ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#Test ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
NewData <- dget(file = "V:/WORK/Sockeye/Chignik/2014 Chignik Inseason/Kyle/Estimates objects/SCHIG14_1_Jun28_Estimates.txt")
Period = 1
NumSampled = 190
NumAnalyzed = 189
Included = 189
...
ChignikInseasonReport.f <- function(NewData = XX, Period = period, NumSampled = sampled, NumAnalyzed = anlayzed, Included = 190, Month = "June", Day = 28) {
  ...
  ## Adding this set of estimates to summary object
  Black2010to2015Means[6, Period] <- NewData$Stats[[1]][1, 1]
  Black2010to2015Uppers[6, Period] <- NewData$Stats[[1]][1, 5]
  Black2010to2015Lowers[6, Period] <- NewData$Stats[[1]][1, 4]
  ...
  plot(1:5, type = "n", axes = FALSE, xlab = "", ylab = "")
  arrows(x0 = 0, y0 = 5, x1 = 5, y1 = 5, length = 0)
  text(x = 4.5, y = 4.75, labels = "90%", adj = 0.5, font = 2, cex = 1.25)
  text(x = 3, y = 4.75, labels = "Stock", adj = 0.5, font = 2, cex = 1.25)
  text(x = c(3, 4.5), y = 4, labels = c("Composition", "Confidence Intervals"), adj = 0.5, font = 2, cex = 1.25)
  text(x = 1, y = 3.25, labels = "Reporting Group", adj = 0, font = 2, cex = 1.25)
  text(x = c(3, 4.2, 4.7), y = 3.25, labels = c("Estimate", "Lower", "Upper"), adj = 0.5, font = 2, cex = 1.25)
  arrows(x0 = 0, y0 = 3, x1 = 5, y1 = 3, length = 0)
  text(x = 1, y = c(2.5, 1.75), labels = c("Black (Early Run)", "Chignik (Late Run)"), adj = 0, cex = 1.25)
  
  ## Change the estimate object name here:
  text(x = 3, y = c(2.5, 1.75), labels = sprintf("%.1f%%", NewData$Stats[[1]][, 1]*100), adj = 0.5, cex = 1.25)
  text(x = 4.2, y = c(2.5, 1.75), labels = sprintf("%.1f%%", NewData$Stats[[1]][, 4]*100), adj = 0.5, cex = 1.25)
  text(x = 4.7, y = c(2.5, 1.75), labels = sprintf("%.1f%%", NewData$Stats[[1]][, 5]*100), adj = 0.5, cex = 1.25)
  ...
  par(cex = 1)
  par(family = 'serif')
  barplot2(height = Black2010to2015Means * 100, names.arg = dimnames(Black2010to2015Means)[[2]], beside = TRUE, col = c("grey90", "grey70", "grey50", "grey30", "grey10", "red"), xlab = "Sample date",
           ylab = "Black (Early Run) percentage of sample", plot.ci = TRUE, ci.l = Black2010to2015Lowers * 100, ci.u = Black2010to2015Uppers * 100, axes = FALSE)
  axis(side = 2, at = seq(0, 100, 25))
  abline(h = 0)
  legend(x = "topright", legend = rownames(Black2010to2015Means), fill = c("grey90", "grey70", "grey50", "grey30", "grey10", "red"), bty = "n")
  abline(h = 50, lwd = 2)
...
# Testing the function
SCHIG15_1_Jun27_Estimates_TEST <- dget(file = "V:/WORK/Sockeye/Chignik/2014 Chignik Inseason/Kyle/Estimates objects/SCHIG14_1_Jun28_Estimates.txt")
SCHIG15_1_Jun27_Estimates_TEST$Stats[[1]][1, c(1, 5, 4)] <- c(0.98, 1, 0.95)
...
# Go reset "Lowers" "Means" and "Uppers"
# Save function
dput(ChignikInseasonReport.f, file = "2015/Objects/ChignikInseasonReportOLD.f.txt")