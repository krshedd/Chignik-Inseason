...
library(kableExtra)
library(ggpubr)
load("../../2019/chignik_2019_run1.RData")
# 
...
sillys_strata_2019_n.df <- as.tibble(cbind(sillys_strata_2019_n, "silly_strata" = rownames(sillys_strata_2019_n))) %>% 
  rename(n = Final) %>% 
...
chignik_2019_dates.bayes %>% 
  ...
  left_join(sillys_strata_2019_n.df) %>% 
...
geom_point(data = filter(chignik_2019_dates.bayes, repunit == "Black Lake"), aes(x = date, y = mean * 100), colour = "red", cex = 5) +
  geom_errorbar(data = filter(chignik_2019_dates.bayes, repunit == "Black Lake"), aes(x = date, y = mean * 100, ymin = `5%` * 100, ymax = `95%` * 100), colour = "red", lwd = 1.5, width = 3) +
  scale_x_date(date_labels = "%B %d", date_breaks = "1 week") +
  xlab("Sample Date") +
  ylab("Black Lake (Early Run) % of Sample") +
  ggtitle("2019 Chignik Sockeye GSI\nwith Expected Transition and 2019 Transition")+
