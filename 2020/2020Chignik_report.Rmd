...
library(kableExtra)
library(ggpubr)
load("../../2020/chignik_2020_run4.RData")
# 
...
Summary of genetic stock composition estimates (mean with 90% CI) for each of the `r nrow(sillys_strata_2020_n)` sampling strata to date.

```{r summary_table, warning=FALSE, message=FALSE}
sillys_strata_2020_n.df <- as_tibble(cbind(sillys_strata_2020_n, "silly_strata" = rownames(sillys_strata_2020_n))) %>% 
...
chignik_2020_dates.bayes %>% 
  ...
  left_join(sillys_strata_2020_n.df, by = c("silly_strata" = "silly")) %>% 
...
```{r stock_comp_figure, fig.height=6.3, fig.width=7.5, fig.align="center"}
mod_avg %>%
  filter(date > "2020-06-15" & date < "2020-07-30") %>%
  ...
  geom_point(
    data = filter(chignik_2020_dates.bayes, repunit == "Black Lake"),
  ...
  ggtitle("2020 Chignik Sockeye GSI\nwith Average Transition (ADF&G)") +
