...
```{r adfg_avg_model}
mod_avg <-
  read_csv(file = "../../2021/2021_preseason_average_GSI_model.csv")
...
PoolCollectionsByDateDF <- function(silly, date.df, loci) {
  sapply(silly, function(mix) {
    mix.dates <-
      unique(as.Date(get(paste(
        mix, ".gcl", sep = ''
      ))$attributes$CAPTURE_DATE))
    by(data = date.df, INDICES = date.df$strata, function(x) {
      IDs <-
        AttributesToIDs.GCL(silly = mix,
                            attribute = "CAPTURE_DATE",
                            matching = mix.dates[mix.dates >= x$begin_date &
                                                   mix.dates <= x$end_date])
      ...
      PoolCollections.GCL(
        ...
        loci = loci,
        IDs = IDs,
        newname = paste(mix, as.character(x$strata), sep = "_")
      )
      list("First Last Fish" = range(as.numeric(unlist(IDs))),
           "n" = get(paste(
             mix, "_", as.character(x$strata), ".gcl", sep = ''
           ))$n)
    })
...
# Strata dates
date_2021.df <- tibble(
  strata = 1:6,  # strata, this is to set up all 6 strata at once!!!
  begin_date = as.Date(
    x = c(
      "2021-06-25",
      "2021-07-01",
...
# Pool
PoolCollectionsByDateDF(silly = sillys,
                        date.df = date_2021.df,
                        loci = loci24_RAD)

# Save
sillys_strata_2021 <- setdiff(str_replace(
  string = objects(pattern = "\\.gcl"),
  pattern = "\\.gcl",
  replacement = ''
),
sillys)
save_objects(objects = "sillys_strata_2021", path = "../../2021/Objects")
save_sillys(sillyvec = sillys_strata_2021, path = "../../2021/Raw genotypes/Strata/")  # sillys_strata

# Verify
sapply(sillys_strata_2021, function(silly) {  # sillys_strata
  table(get(paste0(silly, ".gcl"))$attributes$CAPTURE_DATE, useNA = "always")
})
```
...
original_sillys_strata_2021_n_locus <- SampSizeByLocus.GCL(sillyvec = sillys_strata_2021, loci = loci24_RAD)
min(original_sillys_strata_2021_n_locus)
round(apply(original_sillys_strata_2021_n_locus, 1, function(locus) {min(locus) / max(locus)}), 2)
```

Plot genotyping success by locus and strata.
```{r run1_run6_QA_loci_plot}
original_sillys_strata_percent_locus <- apply(original_sillys_strata_2021_n_locus, 1, function(row) {row / max(row)} )
...
Awesome! Didn't lose very many samples at all this year!!! Even the plate that I cut didn't look too bad!

## Combine loci

Need to combine GDPH and MHC markers
```{r combine_loci_run1_run6}
CombineLoci.GCL(sillyvec = sillys_strata_2021, markerset = c("One_MHC2_251", "One_MHC2_190"), delim = ".", update = TRUE)
CombineLoci.GCL(sillyvec = sillys_strata_2021, markerset = c("One_GPDH2", "One_GPDH"), delim=".", update = TRUE)
```

Save post-QA, combined marker genotypes (i.e. final mixture genotypes).
```{r save_final_genotypes_run1_run6}
if(!dir.exists("../../2021/Raw genotypes/Strata_postQA_loci_combined")) {
  dir.create("../../2021/Raw genotypes/Strata_postQA_loci_combined")
}

save_sillys(sillyvec = sillys_strata_2021, path = "../../2021/Raw genotypes/Strata_postQA_loci_combined/")  # sillys_strata
```
...
Save mixture fortran
```{r mix_fortran}
MixtureFortran <- 
  CreateMixture.GCL(
    sillys = sillys_strata_2021[1], 
    loci = loci22_RAD, 
    IDs = NULL, 
    mixname = sillys_strata_2021[1],
...
Create all 6 mixtures
```{r BAYES_mixtures_run1_run6}
sapply(sillys_strata_2021, function(silly){
  CreateMixture.GCL(
    sillys = silly, 
    loci = loci22_RAD, 
    IDs = NULL, 
...
(
  chignik_late_prior_1 <- mod_avg %>%
    filter(date == date_2021.df$begin_date[1]) %>%
...
CreateControlFile.GCL(
  ...
  basename = "Chignik16pops22RAD_develop", 
  suffix = "", 
  nreps = 40000, 
  nchains = 5,
  groupvec = groupvec16, 
...
trace_plot.f <- function(estimate_obj) {
  ...
  plot(
    estimate_obj[[2]][[1]][seq(from = 1, to = 100000, by = 10), 1],
    type = "l",
    ylim = c(0, 1),
    ylab = "",
    main = "Black Lake",
  ...
  plot(
    estimate_obj[[2]][[1]][seq(from = 1, to = 100000, by = 10), 2],
    type = "l",
    ylim = c(0, 1),
    ylab = "",
    main = "Chignik Lake",
  ...
  print(estimate_obj[[1]][[1]][, c("GR", "mean")])
}
...
Compare to ADF&G pre-season model average
...
mod_avg %>% 
  filter(between(date, date_2021.df$begin_date[strata], date_2021.df$end_date[strata]))
```
...
We lost one fish in *BAYES* from run 2 due to an unrecognized allele (allele 4, CGCC for One_GPDH2.One_GPDH, evidently no one calls blues for One_GPDH2...). Need to update that in the samples size table.

```{r}
LocusControl$alleles$One_GPDH2.One_GPDH[4]
```
...
(
  sillys_strata_2021_n <- sillys_strata_2021_n %>%
    ...
    mutate(final = case_when(
      silly == "SCHIG21_2" ~ as.integer(final - 1),
      TRUE ~ final
    ))
)
...
We lost three additional fish in *BAYES* from run 3 due to an unrecognized allele (1 x allele 2 CCCT and 2 x allele 4 CGCC for One_GPDH2.One_GPDH, evidently no one calls blues for One_GPDH2...). Need to update that in the samples size table.

```{r}
LocusControl$alleles$One_GPDH2.One_GPDH[c(2, 4)]
```
...
(
  sillys_strata_2021_n <- sillys_strata_2021_n %>%
    ...
    mutate(final = case_when(
      silly == "SCHIG21_2" ~ as.integer(final - 1),
      silly == "SCHIG21_3" ~ as.integer(final - 3),
      TRUE ~ final
    ))
)
...
We lost one additional fish in *BAYES* from run 6 due to an unrecognized allele (1 x allele 4 CGCC for One_GPDH2.One_GPDH, evidently no one calls blues for One_GPDH2...). Need to update that in the samples size table.

```{r}
LocusControl$alleles$One_GPDH2.One_GPDH[c(4)]
```
...
(
  sillys_strata_2021_n <- sillys_strata_2021_n %>%
    ...
    mutate(final = case_when(
      silly == "SCHIG21_2" ~ as.integer(final - 1),
      silly == "SCHIG21_3" ~ as.integer(final - 3),
      silly == "SCHIG21_6" ~ as.integer(final - 1),
      TRUE ~ final
    ))
)
...
## Prior for next round
```{r prior_run7}
chignik_7_2021_prior <- Prior.GCL(groupvec = groupvec16, groupweights = SCHIG21_6_estimates[[1]][[1]][, 1], minval = 0.01)
save_objects(objects = "chignik_7_2021_prior", path = "../../2021/Objects/")
```
...
# Summary for 6 Traditional Strata

```{r save_run6}
load("../../2021/chignik_2021_run6.RData")
```
...
(chignik_2021.bayes <-
   bind_rows(sapply(sillys_strata_2021, function(mix) {
...
write_csv(x = chignik_2021_dates.bayes, file = "../../2021/Estimates tables/Chignik 2021 BAYES Estimates.csv")
```

## Save
```{r save_run6}
save.image("../../2021/chignik_2021_run1_run6.RData")
```

Done with the traditional 6 strata, go run the `2021Chignik_report.Rmd` markdown!
...
Damn, they didn't build the date into the 7th strata. No matter, I'll read the ASL that Michelle sent.
...
(
  asl <-
    readxl::read_xlsx(path = "../../2021/ASL/2021 Chignik Genetic Sample ASL.xlsx", sheet = "Sheet1")
)
```

Get min and max vial ranges for each sample date
```{r}
asl %>% 
  group_by(sample_date) %>% 
  summarise(min(dna_vial),
            max(dna_vial))
```

Modify the `attributes`
```{r}
SCHIG21.gcl$attributes$CAPTURE_DATE[1141:1287] <- as.POSIXct(x = "2021-08-08")
SCHIG21.gcl$attributes$CAPTURE_DATE[1288:1330] <- as.POSIXct(x = "2021-08-09")
sapply(sillys, function(silly) {table(get(paste0(silly, ".gcl"))$attributes$CAPTURE_DATE, useNA = "always")} )
```
...
# Strata dates
date_2021.df <- tibble(
  strata = 1:7,  # strata, this is to set up all 6 strata at once!!!
  begin_date = as.Date(
    x = c(
      "2021-06-25",
      "2021-07-01",
...
# Save
save_sillys(sillyvec = sillys_strata, path = "../../2021/Raw genotypes/Strata/")  # sillys_strata

# Verify
sapply(sillys_strata, function(silly) {  # sillys_strata
  table(get(paste0(silly, ".gcl"))$attributes$CAPTURE_DATE, useNA = "always")
})
```

Check `sillys_strata_2021`, to see if it includes SCHIG21_7
```{r}
sillys_strata_2021
```
...
Make a new one
```{r}
sillys_strata_2021_run7 <- setdiff(str_replace(
  string = objects(pattern = "\\.gcl"),
  pattern = "\\.gcl",
  replacement = ''
),
...
```{r run1_run7_QA_loci}
## Get sample size by locus
original_sillys_strata_2021_n_locus <- SampSizeByLocus.GCL(sillyvec = sillys_strata_2021_run7, loci = loci24_RAD)
min(original_sillys_strata_2021_n_locus)
round(apply(original_sillys_strata_2021_n_locus, 1, function(locus) {min(locus) / max(locus)}), 2)
```

Plot genotyping success by locus and strata.
```{r run1_run7_QA_loci_plot}
original_sillys_strata_percent_locus <- apply(original_sillys_strata_2021_n_locus, 1, function(row) {row / max(row)} )
which(apply(original_sillys_strata_percent_locus, 2, min) < 0.8)  # no re-runs!
...
Perform standard data QA processes to filter out untrustworth genotypes.
```{r qa_setup_2021_run1_run7}
sillys_strata_2021_run7_n <- tibble(silly = sillys_strata_2021_run7) %>% 
  mutate(genotyped = sapply(silly, function(x) get(paste0(x, ".gcl"))$n))
```
...
Remove individuals missing >=20% of genotypes (i.e. the 80% rule).
```{r qa_missing_2021_run1_run7}
sillys_strata_2021_run7_MissLoci <- RemoveIndMissLoci.GCL(sillyvec = sillys_strata_2021_run7, proportion = 0.8)
save_objects("sillys_strata_2021_run7_MissLoci", "../../2021/Objects/")

# show individuals removed
sillys_strata_2021_run7_MissLoci[sillys_strata_2021_run7_MissLoci != "None"]

sillys_strata_2021_run7_n <- sillys_strata_2021_run7_n %>% 
  mutate(missing = genotyped - sapply(silly, function(x) get(paste0(x, ".gcl"))$n))
```
...
Remove duplicate individuals within the same collection. Typically we specify *duplicates* as a pair of individuals that share >=95% of genotypes. Once a pair of *duplicates* is identified, we keep the individual with the most genotypes and remove the other.
```{r qa_duplicate_2021_run1_run7}
sillys_strata_2021_run7_DuplicateCheck95MinProportion <-
  CheckDupWithinSilly.GCL(
    sillyvec = sillys_strata_2021_run7,
    loci = loci24_RAD,
    quantile = NULL,
    minproportion = 0.95
  )

(sillys_strata_2021_run7_DuplicateCheckReportSummary <-
  sapply(sillys_strata_2021_run7, function(x)
    sillys_strata_2021_run7_DuplicateCheck95MinProportion[[x]]$report, simplify = FALSE))

sillys_strata_2021_run7_RemovedDups <-
  RemoveDups.GCL(sillys_strata_2021_run7_DuplicateCheck95MinProportion)

save_objects(
  c("sillys_strata_2021_run7_DuplicateCheckReportSummary", "sillys_strata_2021_run7_RemovedDups"),
  "../../2021/Objects/"
)

# show individuals removed
sillys_strata_2021_run7_RemovedDups[sillys_strata_2021_run7_RemovedDups != "Nothing Removed"]
...
How many fish did we end up with in our final baseline? Save final genotypes.
```{r qa_final_2021_run1_run7}
(sillys_strata_2021_run7_n <- sillys_strata_2021_run7_n %>% 
  mutate(final = sapply(silly, function(x) get(paste0(x, ".gcl"))$n)))

save_objects(objects = "sillys_strata_2021_run7_n", path = "../../2021/Objects/")
write_csv(sillys_strata_2021_run7_n, "../../2021/Tables/sillys_strata_2021_run7_n.csv")
```
...
Save post-QA, combined marker genotypes (i.e. final mixture genotypes).
```{r save_final_genotypes_run1_run7}
if(!dir.exists("../../2021/Raw genotypes/Strata_postQA_loci_combined")) {
  dir.create("../../2021/Raw genotypes/Strata_postQA_loci_combined")
}

save_sillys(sillyvec = sillys_strata, path = "../../2021/Raw genotypes/Strata_postQA_loci_combined/")  # sillys_strata
```
...
We lost one additional fish in *BAYES* from run 7 due to an unrecognized allele (1 x allele 4 CGCC for One_GPDH2.One_GPDH, evidently no one calls blues for One_GPDH2...). Need to update that in the samples size table.

```{r}
LocusControl$alleles$One_GPDH2.One_GPDH[c(4)]
```

```{r samples_size_update_run7}
(
  sillys_strata_2021_run7_n <- sillys_strata_2021_run7_n %>%
    mutate(final = sapply(silly, function(x)
      get(paste0(
    ...
    mutate(final = case_when(
      ...
      silly == "SCHIG21_3" ~ as.integer(final - 3),
      silly == "SCHIG21_6" ~ as.integer(final - 1),
      silly == "SCHIG21_7" ~ as.integer(final - 1),
      TRUE ~ final
    ))
)
...
save_objects(objects = "SCHIG21_7_estimates", path = "../../2021/Estimates objects/")
```

View traceplot
```{r traceplot_run7}
trace_plot.f(estimate_obj = SCHIG21_7_estimates)
```
...
## Save
```{r save_run7}
save.image("../../2021/chignik_2021_run7.RData")
```


# Summary for all 7 Strata

```{r save_run7}
load("../../2021/chignik_2021_run7.RData")
```
...
(
  SCHIG21_estimates <-
    CustomCombineBAYESOutput.GCL(
      ...
      groupnames = ChignikGroups,
      maindir = "../../2021/BAYES/Output",
      mixvec = sillys_strata_2021_run7,
      prior = "",
      ext = "RGN",
      nchains = 5,
      burn = 0.5,
...
Bind everything together
```{r summary_all_year_run7}
(chignik_2021.bayes <-
   bind_rows(sapply(sillys_strata_2021_run7, function(mix) {
     as_tibble(cbind(SCHIG21_estimates[[mix]], "repunit" = ChignikGroups, silly = mix))
   }, simplify = FALSE)))
```
...
```{r attributes_run7}
SCHIG21_attributes <-
  as_tibble(bind_rows(lapply(sillys_strata_2021_run7, function(silly) {
    cbind(get(paste0(silly, ".gcl"))$attributes, "new_silly" = silly)
  })))
...
```{r}
table(SCHIG21_7.gcl$attributes$CAPTURE_DATE)
```

## Save
```{r save_run7}
save.image("../../2021/chignik_2021_run1_run7.RData")
```

Done with all 7 strata, **no need** to go run the `2021Chignik_report.Rmd` markdown!